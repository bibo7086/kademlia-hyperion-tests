...
find_modes = [0]  # List of find modes
...
def run_sim(config_file, size, seed, find_mode, traffic_step, observer_step):
    try:
        ...
        shutil.move(os.path.join(unix_log_dir, f'average_{size}_{traffic_step}_{find_mode}.csv'), os.path.join(log_dir_config, f"average_{size}_{find_mode}_{traffic_step}.csv"))
...
def main() -> int:
    ...
    # traffic_steps = [72, 36, 24]
    # observer_steps = [71,35, 23]
