...
public class RoutingTable implements Cloneable {
  ...
  // New Add a neighbour
  // Add neighbor to the correct k-bucket wit latency awareness
  public boolean addNeighbour(E2ETransport transport, Node src, Node dest, BigInteger node) {
    ...
    if (findMode == 0 || findMode == 1) {
      ...
      int prefix_len = Util.prefixLen(nodeId, node);
      // Add the node to the k-bucket
      return bucketAtDistancexor(prefix_len).addNeighbour(transport, src, dest, node);
    } else {
      int distance = Util.logDistance(nodeId, node);
      return bucketAtDistance(distance).addNeighbour(transport, src, dest, node);
    }
  }
