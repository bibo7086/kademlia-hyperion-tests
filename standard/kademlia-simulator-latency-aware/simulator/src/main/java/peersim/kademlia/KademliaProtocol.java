...
public class KademliaProtocol implements Cloneable, EDProtocol {
  ...
  private void handleResponse(Message m, int myPid) {
    ...
    if (m.src != null) {
      ...
      Node dest = nodeIdtoNode(m.src.getId());

      // destpid = dest.getKademliaProtocol().getProtocolID();

      // Get the transport protocol
      // transport = (UnreliableTransport) (Network.prototype).getProtocol(tid);
      transport = (E2ETransport) (Network.prototype).getProtocol(tid);

      // Add destination node to routing table
      routingTable.addNeighbour(transport, src, dest, m.src.getId());
      // int destpid;
    ...
    if (fop != null) {
      ...
      for (BigInteger neighbor : neighbors) {
        Node dest = nodeIdtoNode(neighbor);
        routingTable.addNeighbour(transport, src, dest, neighbor);
  ...
  private void sendMessage(Message m, BigInteger destId, int myPid) {
    ...
    // Add destination node to routing table
    this.routingTable.addNeighbour(transport, src, dest, destId);
    // int destpid;
